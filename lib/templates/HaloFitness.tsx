...
const features = [
  {
    icon: "⚡",
    title: "AI 코치",
    desc: "내 몸 상태에 맞춰 매일 운동 강도를 자동 조정",
  },
  {
    icon: "📊",
    title: "심박·수면·회복",
    desc: "워치 연동으로 회복까지 점수화",
  },
  {
    icon: "🎯",
    title: "30일 챌린지",
    desc: "함께 시작한 친구와 매일 인증",
  },
];
...
export default function HaloFitness() {
  return (
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-6 py-20 md:grid-cols-[1.1fr_1fr] md:py-28">
          <div>
            ...
            <p className="mt-7 max-w-md text-lg text-white/70">
              AI 트레이너가 내 몸 상태를 매일 학습합니다. 30일 챌린지로 진짜 변화를 시작하세요.
            </p>
          ...
          <div className="relative mx-auto aspect-[9/16] w-full max-w-xs overflow-hidden rounded-[2.5rem] border-[6px] border-white/10 shadow-2xl shadow-[#ff3d8b]/40">
            ...
            <div className="absolute inset-x-4 bottom-4 rounded-2xl bg-black/80 p-4 backdrop-blur">
              <div className="flex items-center justify-between text-xs">
                <span className="text-white/60">오늘의 미션</span>
                <span className="rounded-full bg-[#ffb44a] px-2 py-0.5 text-[10px] font-bold text-black">+150 XP</span>
              </div>
              <p className="mt-2 font-bold">HIIT 25분 — 심박존 4 유지</p>
      ...
      <section className="bg-gradient-to-b from-transparent via-[#ff3d8b]/5 to-transparent py-20">
        <div className="mx-auto max-w-6xl px-6">
          <h2 className="text-center text-4xl font-black md:text-5xl">진짜로 바꾼 사람들</h2>
          <div className="mt-14 grid gap-5 md:grid-cols-3">
            {testimonials.map((t) => (
              <div key={t.handle} className="rounded-3xl bg-white/5 p-6 backdrop-blur">
                <Image
                  src={`https://i.pravatar.cc/120?img=${t.img}`}
                  alt={t.handle}
                  width={56}
                  height={56}
                ...
                <p className="mt-5 text-xs text-[#ffb44a]">{t.handle}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-3xl px-6 py-24 text-center">
        ...
        <p className="mt-6 text-white/60">7일 무료. 언제든 해지. 카드 등록 없이 시작하세요.</p>
      ...
      <footer className="border-t border-white/10 py-8 text-center text-xs text-white/40">
        © 2025 HALO Fitness. Built for the everyday athlete.
      </footer>
    </div>
  );
}
